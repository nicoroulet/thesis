...
def build_unet(n_classes, depth=4, base_filters=32):
  """ Builds a UNet neural network and returns the input and output layers.

  Args:
    n_classes: number of output classes (labels).
    depth: number of UNet layers. This is not the number of layers of the
      neural network, a UNet layer is the combination of convolution layers
      and maxpooling/upconvolution layers.
    base_filters: number of filters on the initial layers. The number of filters
      is doubled on each maxpooling and halved on each upconvolution .
  Return:
...
class UNet(Model):
  def __init__(self, n_classes, depth=4, base_filters=32):
    inputs, outputs = build_unet(n_classes, depth, base_filters)
